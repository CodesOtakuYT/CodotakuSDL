...
namespace codotaku {
    ...
    public:
        ...
        void bindVertexBuffers(Uint32 firstSlot, std::span<SDL_GPUBufferBinding> bindings) noexcept {
            SDL_BindGPUVertexBuffers(pass_, firstSlot, bindings.data(), bindings.size());
        }
        ...
        void drawIndexed(
            ...
            Uint32 firstInstance = 0) noexcept {
    ...
    inline void RenderPass::bindIndexBuffer(const Buffer &buffer, Uint32 offset,
                                            SDL_GPUIndexElementSize elementSize) noexcept {
        SDL_GPUBufferBinding binding{.buffer = buffer.handle(), .offset = offset};
